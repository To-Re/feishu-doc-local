...
export function ContentSyncControls({ busy, disabled, onPreview }: Pick<Props,'busy'|'disabled'|'onPreview'>) {
  return <div className="content-sync-control" aria-label="正文同步">
    <button className="content-sync-main" disabled={busy || disabled} onClick={onPreview}><GitCompareArrows size={15}/>预览同步</button>
  </div>;
}
export function ContentSync({ project, direction, open, busy, disabled, preview, stale, error, message, onDirection, onPreview, onExecute, onClose }: Props) {
  ...
  return <>
    {!open && error && <p className="content-sync-status project-error" role="alert">{error}</p>}
    {!open && message && <p className="content-sync-status" role="status">{message}</p>}
    {open && <div className="modal-overlay content-sync-modal" onKeyDown={event => { if (event.key === 'Escape' && !busy) { event.stopPropagation(); onClose(); } }}>
      <section ref={dialog} tabIndex={-1} className="content-sync-dialog" role="dialog" aria-modal="true" aria-labelledby="content-sync-title" onKeyDown={containDialogFocus}>
        ...
        <div className="content-sync-decision">
          <SegmentedControl label="预览方向" value={direction} disabled={busy || disabled} options={syncDirectionOptions} onChange={onDirection}/>
          <div className="project-dialog-actions">
            {stale || !preview ? <button className="primary" disabled={busy || disabled} onClick={onPreview}>{busy?'正在预览…':'重新预览'}</button> : preview.status !== 'equal' && presentation &&
              <button className="primary" disabled={busy || disabled || preview.direction!==direction} onClick={onExecute}>{busy ? '正在同步…' : presentation.confirm}</button>}
          </div>
        </div>
        ...
        {!preview && busy && <p role="status">正在读取两端内容，尚未执行同步。</p>}
        {preview && presentation && <><h3>{presentation.operation}</h3>
        <p>{preview.summary}</p></>}
        ...
        {preview && presentation && <>{preview.status!=='equal'&&<p className="project-help">{presentation.description}评论单独同步。</p>}
