...
describe('trusted CLI process lifecycle',()=>{
  ...
  it.each(['relative/document','/document\0invalid'])('rejects an invalid per-write working directory before spawning',async cwd=>{
    const execute=vi.fn(),runner=new ManagedCLIRunner(execute as unknown as typeof spawn,{cwd:'/trusted/vault'});
    await expect(runner.run('/configured/lark-cli',[],{cwd})).rejects.toThrow('仓库绝对路径');expect(execute).not.toHaveBeenCalled();
  });
