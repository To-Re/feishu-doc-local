...
4. 执行命令“本地飞书文档：打开飞书 XML 文档”，或右键 XML 选择“用本地飞书文档打开”。打开前会校验 DocxXML；普通配置 XML、HTML、SVG 不会进入阅读器，也不会生成评论文件。

插件不注册所有 `.xml` 的默认阅读器，普通 XML 继续由 Obsidian 或其他插件处理。文件树不显示 XML 时，可开启 Obsidian 的“检测所有文件扩展名”，或使用本插件命令选择文件。命令面板还提供“新建本地文档”和“恢复未保存草稿”。DocxXML 是可交换格式，校验只判断结构，不证明文档来自飞书；仅含 `<p>` 等共有标签的片段也可能是有效 DocxXML。
